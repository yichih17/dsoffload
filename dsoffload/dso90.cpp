...
double get_T_constraint90(BS *b);
double predict_constraint90(BS *b, UE *u);

void joinBS90(UE* u, BS* targetBS, double T)
{
	if (u->connecting_BS != NULL)
	{
		...
		u->connecting_BS->systemT_constraint = get_T_constraint90(u->connecting_BS);
	}
	...
	targetBS->systemT_constraint = get_T_constraint90(targetBS);
}

bool join_minT_bs90(UE* u, vector <BS*> *list, vector <double> *list_T)
{
	BS* targetBS = NULL;
	double minT = 0;
...
bool findbs_dso90(UE* u, connection_status* cs, int depth, int depth_max)
{
	...
	for (int i = 0; i < u->availBS.size(); i++)
	{
		...
		{
			if (T > predict_constraint90(u->availBS.at(i), u))	//T上限飽和
			{
				saturated_bs.push_back(u->availBS.at(i));
				saturated_bs_T.push_back(T);
			}
	...
	{
		if (join_minT_bs90(u, &no_influence_bs, &no_influence_bs_T))
			return true;
		else
			return false;
	}
	else
	{
		if (depth == depth_max)								//從受影響基地台擇一
		{
			if (influence_bs.size() == 0)
				return false;
			if (join_minT_bs90(u, &influence_bs, &influence_bs_T))
				return true;
			else
				return false;
		}
				...
				*cs = cs_origin;
				...
				for (int j = 0; j < ue_sorted.size(); j++)
				{
					...
					if (findbs_dso(ue_sorted.at(j), cs, depth + 1, depth_max))
					{
						...
						if (T < predict_constraint90(offload_bs.at(i), u) && all_ue_satisfy(offload_bs.at(i), T))
							break;
					}
				}
				if (T == -1)
					continue;
				if (T > predict_constraint90(offload_bs.at(i), u))
					continue;
...
double get_T_constraint90(BS *b)
{
	...
	if (type_conut[2] >= b->connectingUE.size() * 0.9)
		return 300;
	else
	{
		if (type_conut[2] + type_conut[1] >= b->connectingUE.size() * 0.9)
			return 100;
		else
			return 50;
	}
...
double predict_constraint90(BS *b, UE *u)
{
	...
	if (type_conut[2] >= (b->connectingUE.size() + 1) * 0.9)
		return 300;
	else
	{
		if (type_conut[2] + type_conut[1] >= (b->connectingUE.size() + 1) * 0.9)
			return 100;
		else
			return 50;
	}
